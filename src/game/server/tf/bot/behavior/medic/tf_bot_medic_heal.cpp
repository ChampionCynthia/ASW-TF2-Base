...
bool CTFBotMedicHeal::IsStable( CTFPlayer *patient ) const
{
	...
	if ( patient->GetTimeSinceLastInjury( patient->GetTeamNumber() == TF_TEAM_BLUE ? TF_TEAM_RED : TF_TEAM_BLUE ) < safeTime )
		return false;
...
//
bool CTFBotMedicHeal::IsReadyToDeployUber( CWeaponMedigun* pMedigun ) const
{
	if( !pMedigun )
		return false;

	if ( pMedigun->GetChargeLevel() < 1.0f )
		return false;
	...
	if ( medigun )
	{
		...
		if ( IsReadyToDeployUber( medigun ) && CanDeployUber( me, medigun ) )
		{
...
#endif
			{
				...
				{
					if ( me->GetTimeSinceLastInjury( me->GetTeamNumber() == TF_TEAM_BLUE ? TF_TEAM_RED : TF_TEAM_BLUE ) < 1.0f /*|| TFGameRules()->IsMannVsMachineMode()*/ )
					{
						useUber = true;
					}
				}
