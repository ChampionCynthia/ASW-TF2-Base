...
ActionResult< CTFBot >	CTFBotSpyAttack::Update( CTFBot *me, float interval )
{
	...
	if ( me->m_Shared.InCond( TF_COND_STEALTHED ) && m_decloakTimer.IsElapsed() )
	{
	...
	if ( /*me->IsThreatAimingTowardMe( playerThreat, 0.99f ) &&*/ me->GetTimeSinceLastInjury( ( me->GetTeamNumber() == TF_TEAM_BLUE ? TF_TEAM_RED : TF_TEAM_BLUE ) ) < 1.0f )
	{
