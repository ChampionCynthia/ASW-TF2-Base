...
void CreateBotName( int iTeam, int iClassIndex, CTFBot::DifficultyType skill, char* pBuffer, int iBufferSize )
{
	...
	// TF2008 has no training. -Cynthia
#if 0
...
void CTFBot::PressFireButton( float duration )
{
	...
	if ( !BaseClass::CanAttack() || HasAttribute( CTFBot::SUPPRESS_FIRE ) )
	{
		ReleaseFireButton();
		return;
	}
...
const char *CTFBot::GetNextSpawnClassname( void ) const
{
	...
	// Comp. and KOTH don't exist in TF2008! -Cynthia
#if 0
...
void CTFBot::Touch( CBaseEntity *pOther )
{
	...
	{
		if ( them->m_Shared.InCond( TF_COND_STEALTHED ) || them->m_Shared.InCond( TF_COND_DISGUISED ) )
		{
...
CCaptureFlag *CTFBot::GetFlagToFetch( void ) const
{
	...
	for ( int i=0; i<ICaptureFlagAutoList::AutoList().Count(); ++i )
	{
		...
		if ( const_cast< CTFBot * >( this )->HasTheFlag() )
		{
		...
		int enemyTeam = ( GetTeamNumber() == TF_TEAM_BLUE ? TF_TEAM_RED : TF_TEAM_BLUE );
...
bool CTFBot::IsAmmoLow( void ) const
{
	...
	if ( myWeapon )
	{
		...
		float ratio = (float)GetAmmoCount( TF_AMMO_PRIMARY ) / (float)( const_cast< CTFBot * >( this )->GetPlayerClass()->GetData()->m_aAmmoMax[ TF_AMMO_PRIMARY ] );
...
bool CTFBot::IsAmmoFull( void ) const
{
	...
	bool isPrimaryFull = GetAmmoCount( TF_AMMO_PRIMARY ) >= const_cast< CTFBot * >( this )->GetPlayerClass()->GetData()->m_aAmmoMax[ TF_AMMO_PRIMARY ];
	bool isSecondaryFull = GetAmmoCount( TF_AMMO_SECONDARY ) >= const_cast< CTFBot * >( this )->GetPlayerClass()->GetData()->m_aAmmoMax[ TF_AMMO_SECONDARY ];
...
void CTFBot::UpdateLookingAroundForIncomingPlayers( bool lookForEnemies )
{
	...
	if ( myArea )
	{
		...
		if ( !lookForEnemies )
		{
			team = ( team == TF_TEAM_BLUE ) ? TF_TEAM_RED : TF_TEAM_BLUE;
		}
...
// But airblast doesn't exist here...
#if 0
//---------------------------------------------------------------------------------------------
//
...
CTFPlayer *CTFBot::SelectRandomReachableEnemy( void )
{
	...
	CollectPlayers( &livePlayerVector, ( GetTeamNumber() == TF_TEAM_BLUE ? TF_TEAM_RED : TF_TEAM_BLUE ), COLLECT_ONLY_LIVING_PLAYERS );
...
//-----------------------------------------------------------------------------
Vector CTFBot::EstimateProjectileImpactPosition( float pitch, float yaw, float initVel )
{
...
// do.
//-----------------------------------------------------------------------------
Vector CTFBot::EstimateProjectileImpactPosition( CTFWeaponBaseGun *weapon )
{
	if ( !weapon )
	{
		return GetAbsOrigin();
...
//-----------------------------------------------------------------------------
Vector CTFBot::EstimateStickybombProjectileImpactPosition( float pitch, float yaw, float charge )
{
...
CTeamControlPoint *CTFBot::SelectClosestControlPointByTravelDistance( CUtlVector< CTeamControlPoint * > *pointVector ) const
{
...
bool CTFBot::IsThreatAimingTowardMe( CBaseEntity *threat, float cosTolerance ) const
{
	...
	if ( player == NULL )
	{
		...
		{
			// are we in range?
			if ( threatRange < 1100.0f ) // Should be a #define, I know.
			{
...
//-----------------------------------------------------------------------------
bool CTFBot::IsAnyEnemySentryAbleToAttackMe( void )
{
	...
	for ( int i=0; i<IBaseObjectAutoList::AutoList().Count(); ++i )
	{
		...
		// are we in range?
		// Cynthia: Range check performed by IsThreatAimingTowardMe()
