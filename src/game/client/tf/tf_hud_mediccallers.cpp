...
void CTFMedicCallerPanel::PaintBackground( void )
{
	...
	Vector vecDelta = vecTarget - MainViewOrigin( iSlot );
	...
	if( !bOnscreen || iX < halfWidth || iX > ScreenWidth()-halfWidth )
	{
		...
		if( MainViewRight( iSlot ).Dot( vCenter - MainViewOrigin( iSlot ) ) > 0 )
		{
	...
	else
	{
		...
		trace_t	tr;
		UTIL_TraceLine( vecTarget, MainViewOrigin( iSlot ), MASK_OPAQUE, NULL, COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction >= 1.0f )
		{
			m_bOnscreen = true;
			SetAlpha( 0 );
