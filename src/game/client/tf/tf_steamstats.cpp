...
void CTFSteamStats::FireGameEvent( IGameEvent *event )
{
	...
	{
		CSteamAPIContext *pSteamAPIContext = engine->GetSteamAPIContext();
		Assert( pSteamAPIContext );
		if ( !pSteamAPIContext )
			return; 

		ISteamUserStats *pSteamUserStats = pSteamAPIContext->SteamUserStats();
		Assert( pSteamUserStats );
		if ( !pSteamUserStats )
			return;
		...
		for ( int iClass = TF_FIRST_NORMAL_CLASS; iClass <= TF_LAST_NORMAL_CLASS; iClass++ )
		{
			...
			for ( int iStat = 0; iStat < ARRAYSIZE( g_SteamStats ); iStat++ )
			{
				...
				if ( pSteamUserStats->GetStat( szStatName, &iData ) )
				{
...
void CTFSteamStats::UploadStats()
{
	...
	for ( int iClass = TF_FIRST_NORMAL_CLASS; iClass <= TF_LAST_NORMAL_CLASS; iClass++ )
	{
		...
		for ( int iStat = 0; iStat < ARRAYSIZE( g_SteamStats ); iStat++ )
		{
			...
			Q_snprintf( szStatName, ARRAYSIZE( szStatName ), "%s.accum.%s", g_aPlayerClassNames_NonLocalized[iClass], g_SteamStats[iStat].pszName );
			pSteamUserStats->SetStat( szStatName, classStats.accumulated.m_iStat[g_SteamStats[iStat].iStat] );

			Q_snprintf( szStatName, ARRAYSIZE( szStatName ), "%s.max.%s", g_aPlayerClassNames_NonLocalized[iClass], g_SteamStats[iStat].pszName );
			pSteamUserStats->SetStat( szStatName, classStats.max.m_iStat[g_SteamStats[iStat].iStat] );
		}
	}
