...
void C_TFRagdoll::CreateTFRagdoll()
{
	...
	{
		ClientLeafSystem()->SetTranslucencyType( GetRenderHandle(), RENDERABLE_IS_TRANSLUCENT );
	}		
	...
	if ( IsLocalPlayer() )
	{
		...
		if ( m_iOldTeam != GetTeamNumber() )
		{
			...
			if ( IsX360() )
			{
				...
				/*if ( pTeam )
				{
					// Internal engine code, unavailable in ASW.
...
void C_TFPlayer::UpdateIDTarget()
{
	...
	VectorMA( MainViewOrigin( iSlot ), MAX_TRACE_LENGTH, MainViewForward( iSlot ), vecEnd );
	VectorMA( MainViewOrigin( iSlot ), 10,   MainViewForward( iSlot ), vecStart );
...
RenderableTranslucencyType_t C_TFPlayer::ComputeTranslucencyType( void )
{
	RenderableTranslucencyType_t ret = BaseClass::ComputeTranslucencyType();
...
{
	C_TFPlayer *pPlayer = NULL;
	pPlayer->ComputeTranslucencyType();
}
