	...
	: EditablePanel( NULL, "StatPanel" ), CHudElement( pElementName )
{
	...
	IClientMode *pClientMode = GetFullscreenClientMode();
	Assert( pClientMode );
	...
	m_flTimeLastSpawn = 0;
	vgui::Panel *pParent = pClientMode->GetViewport();
	SetParent( pParent );
...
void CTFStatPanel::WriteStats( void )
{
	...
	if( pSteamAPIContext )
	{
		ISteamUser *pSteamUser = pSteamAPIContext->SteamUser();
		if ( pSteamUser )
		{
...
bool CTFStatPanel::ReadStats( void )
{
	...
	ISteamUser *pSteamUser = NULL;
	CSteamAPIContext *pSteamAPIContext = engine->GetSteamAPIContext();
	if( pSteamAPIContext )
		pSteamUser = pSteamAPIContext->SteamUser();
	...
	if ( ( iCRC == iCRCFile ) && ( iSteamID > 0 ) && pSteamUser ) 
	{		
