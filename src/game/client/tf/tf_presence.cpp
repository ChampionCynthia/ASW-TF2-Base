...
void CTF_Presence::LevelInitPreEntity( void )
{
	...
	if ( pMapName )
	{
		XUSER_CONTEXT xc = { CONTEXT_SCENARIO, GetMapID( pMapName ) };
		UserSetContext( XBX_GetActiveUserId(), xc, true );
	}
}
...
void CTF_Presence::FireGameEvent( IGameEvent *event )
{
	...
	else if ( !Q_stricmp( "controlpoint_initialized", eventname ) )
	{
		...
		XUSER_PROPERTY xpPoints = { PROPERTY_CAPS_TOTAL, sizeof(int), nPoints };
		XUSER_PROPERTY xpOwned = { PROPERTY_CAPS_OWNED, sizeof(int), nOwned };
		UserSetProperty( XBX_GetActiveUserId(), xpPoints, true );
		UserSetProperty( XBX_GetActiveUserId(), xpOwned, true );
	}
	...
	else if ( !Q_stricmp( "ctf_flag_captured", eventname ) )
	{
		...
		if ( pLocalTeam )
		{
			...
			XUSER_PROPERTY xpPlayerTeamScore = { PROPERTY_PLAYER_TEAM_SCORE, sizeof(int), iTeamScore };
			XUSER_PROPERTY xpOpponentTeamScore = { PROPERTY_OPPONENT_TEAM_SCORE, sizeof(int), iOtherScore };
			UserSetProperty( XBX_GetActiveUserId(), xpPlayerTeamScore, true );
			UserSetProperty( XBX_GetActiveUserId(), xpOpponentTeamScore, true );
#if defined ( _X360 )
