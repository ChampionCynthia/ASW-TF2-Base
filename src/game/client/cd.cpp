...
void CCDAudio::_Pause( int, int )
{
	...
	mciGenericParms.dwCallback = (DWORD)GetActiveWindow();
...
void CCDAudio::_Play( int track, int looping )
{
    ...
    mciPlayParms.dwCallback = (DWORD)GetActiveWindow();
...
//-----------------------------------------------------------------------------
void CCDAudio::CD_f ( const CCommand &args )
{
	const char *command;
	int		ret;
	int		n;
	...
	{
		Play( atoi(args[2] ), false );
		return;
	}

	if (stricmp(command, "loop") == 0)
	{
		Play( atoi( args[ 2 ]), true );
		return;
	}
...
int CCDAudio::Init( void )
{
	...
	dwReturn = mciSendCommand(m_uiDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, (DWORD)(LPVOID) &mciSetParms);
    if ( dwReturn )
    {
...
{
	cdThread->AddThreadItem( &CCDAudio::_CDReset, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_Stop, 0, 0 );
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_Pause, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_Eject, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_CloseDoor, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_GetAudioDiskInfo, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_Play, track, looping ? 1 : 0 );
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_Resume, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_SwitchToLauncher, 0, 0);
}
...
{
	cdThread->AddThreadItem( &CCDAudio::_SwitchToEngine, 0, 0);
}
...
void CCDAudio::Frame( void )
{
	...
	cdThread->AddThreadItem( &CCDAudio::_CDUpdate, 0, 0 );
}