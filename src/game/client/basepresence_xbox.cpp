...
//-----------------------------------------------------------------------------
void CBasePresence::UserSetContext( uint nUserIdx, XUSER_CONTEXT const &xc, bool bAsync, AsyncHandle_t *pHandle )
{
	if ( !xboxsystem->UserSetContext( nUserIdx, xc, bAsync, pHandle ) )
	{
...
//-----------------------------------------------------------------------------
void CBasePresence::UserSetProperty( uint nUserIndex, XUSER_PROPERTY const &xp, bool bAsync, AsyncHandle_t *pHandle )
{
 	if ( !xboxsystem->UserSetProperty( nUserIndex, xp, bAsync, pHandle ) )
 	{
...
void CBasePresence::DebugUserSetContext( const CCommand &args )
{
	if ( args.ArgC() == 3 )
	{
		XUSER_CONTEXT xc = { atoi( args.Arg( 1 ) ), atoi( args.Arg( 2 ) ) };
		UserSetContext( XBX_GetActiveUserId(), xc );
	}
	else
	{
...
void CBasePresence::DebugUserSetProperty( const CCommand &args )
{
	if ( args.ArgC() == 3 )
	{
		XUSER_PROPERTY xp = { strtoul( args.Arg( 1 ), NULL, 0 ), sizeof(int), atoi( args.Arg( 2 ) ) };
		UserSetProperty( XBX_GetActiveUserId(), xp );
	}
	else
	{
